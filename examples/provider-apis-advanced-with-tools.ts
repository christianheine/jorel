...
import {AnthropicProvider, GoogleVertexAiProvider, GrokProvider, GroqProvider, LlmToolKit, OllamaProvider, OpenAIProvider} from "../src";
...
const main = async () => {
  ...
  let response = await provider.generateResponse(model, [{
    role: "user",
  ...
  if (response.role === "assistant") {
    ...
    const followUpResponse = await provider.generateResponse(model, [{
      role: "user",
      content: "What's the current temperature in Sydney?"
    },
      response,
